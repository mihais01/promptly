...
static promptly_result_t parse_extended(PROMPTLY_CTX, const char ch) 
{
    ...
    switch (ch) {
        case LEFT_ARROW: {
            ...
            if(ctx->line_wpos > 0) {
                ctx->line_wpos--;
                move_cursor_left(ctx, 1); /* Move cursor left */
            }
            else {
        ...
        case RIGHT_ARROW:
            if(ctx->line_wpos < ctx->line_size) {
                ctx->line_wpos++;
                move_cursor_right(ctx, 1); /* Move cursor right */
            }
            else {
...
promptly_result_t promptly_edit_line(PROMPTLY_CTX, const char ch)
{
    ...
    case PROMPTLY_REQ_DSR: {
        PROMPTLY_WRITE(ctx, "\x1b[6n", 4); /* Request Device Status Report */
